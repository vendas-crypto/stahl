...
def carregar_dados_da_nuvem(nome_aba, tipo="orcar"):
    try:
        ...
        # Mapeia dinamicamente os nomes internos para exibição em português
        df['Situação'] = df['Situação'].map(lambda x: TRADUCAO_SITUACAO.get(str(x).strip(), x))
